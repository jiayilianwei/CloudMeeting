...
namespace OMCS.Boost.MultiChat
{
    ...
    {       
        ...
        {             
            ...
            int i = this.chatGroup.GetOtherMembers().Count;
            foreach (IChatUnit unit in this.chatGroup.GetOtherMembers())
            {
                if (unit.MemberID == "_server")
                {
                    continue;
                }
            ...
            this.groupBox_members.Text = string.Format("Member List（{0}人）" ,this.flowLayoutPanel1.Controls.Count);
